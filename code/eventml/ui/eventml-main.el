...
(defvar eventml-output-buffer "*eventml-output*"
  "Name of the buffer used to display E#'s output.")
...
(defvar eventml-help-buffer "*eventml-help*"
  "Name of the buffer used to display E#'s help menu.")
...
(defvar eventml-nuprl-defs-temp nil
  "Name of the file that contains the E# type of some of Nuprl abstractions.")
...
;; Same as sml-tes-trace but displays E# instead of SML-TES
...
(defun eventml-compile-to-nuprl ()
  "Compiles the E# code into NuPrl."
  (interactive)
...
(defun eventml-simulator (&optional arg)
  ...
  (let ((tmp-udb  use-dialog-box)
    ...
    (let ((command (format "%s\"%s\" \"%s\" -lib \"%s\" -nuprl-defs \"%s\" -simul \"%s\""
			   eventml-cmd-pref
			   (expand-file-name eventml-bin-name       eventml-bin-directory)
			   (buffer-file-name)
			   (expand-file-name eventml-nuprl-defs-esh eventml-lib-directory)
...
(defun eventml-define-keybindings-in-map (map)
  "Sets up key bindings for functions to use the E# type inferencer."
  ;;
  ;; In all the following key bindings, e stands for E#
  ;;
...
(eval-after-load (expand-file-name "eventml-mode.el" eventml-emacs-directory)
  '(easy-menu-define eventml-mode-menu eventml-mode-map "Menu used in `E#-mode'."
     '("EventML"
       ("Slicing"
