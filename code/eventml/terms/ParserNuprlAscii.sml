...
fun parseStream prt lst file inputStream =
...
fun parseFiles _ _ [] = []
  | parseFiles prt lst (file :: files) =
    let val instr = TextIO.openIn file
	val progs = parseStream prt lst file instr
	val _     = TextIO.closeIn instr
    in progs @ (parseFiles prt lst files)
    end

fun parse prt lst file split =
    let val _      = cond_print prt ("[start parsing]\n")
	val timer  = startTimer ()
	val files  = getFiles prt file split
	val terms  = parseFiles prt lst files
	val time   = getSecTime timer
