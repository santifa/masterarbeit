...
type eval_sig = (T.nuprl_term * T.nuprl_term) list
		-> int
		-> T.nuprl_term
		-> T.nuprl_term * int
...
val term3 =
	...
	val bt1   = ("x", T.mk_nuprl_add_term (T.mk_variable_term "x") (T.mk_nuprl_small_integer_term 4))
	val bt2   = ("y", T.mk_nuprl_add_term (T.mk_variable_term "y") (T.mk_nuprl_small_integer_term 5))
...
val term6 =
	...
	val conscase2 = T.mk_nuprl_add_term (T.mk_variable_term "x") (T.mk_variable_term "r")
...
val term7 =
    ...
    in (name, T.mk_nuprl_callbyvalue_term arg ("x", T.mk_variable_term "x"))
    end

val term8 =
    let val arg  = T.mk_pair_term (#2 term3) (T.mk_nuprl_small_integer_term 0)
	val name = "term8: simple example involving callbyvalueall"
    in (name, T.mk_nuprl_callbyvalueall_term arg ("x", T.mk_variable_term "x"))
    end
...
val term10 =
	...
	val sub  = T.mk_nuprl_subtract_term idy (T.mk_nuprl_small_integer_term 1)
	val app  = T.mk_apply_term idr sub
	val add  = T.mk_nuprl_add_term app (T.mk_nuprl_small_integer_term 3)
	...
	val cbva = T.mk_nuprl_callbyvalueall_term appA ("x", T.mk_variable_term "x")
...
val term11 =
	...
	val cbva = T.mk_nuprl_callbyvalueall_term app ("x", T.mk_variable_term "x")
...
val term12 =
	...
	val cbva = T.mk_nuprl_callbyvalueall_term inj ("x", T.mk_variable_term "x")
...
val term13 =
	...
	val cbva = T.mk_nuprl_callbyvalueall_term pair ("x", T.mk_variable_term "x")
...
val term14 =
	...
	val add  = T.mk_nuprl_add_term idr appz
...
val term19 =
	...
	val cbva = T.mk_nuprl_callbyvalueall_term appr ("x", T.mk_variable_term "x")
...
val term21 =
	...
	val idr  = T.mk_variable_term "r"
	val idw  = T.mk_variable_term "w"
	val lamw = T.mk_lambda_term "w" (T.mk_nuprl_add_term idr idy)
	val lamy = T.mk_lambda_term "y" lamw
	val appy = T.mk_apply_term lamy (T.mk_nuprl_small_integer_term 2)
	val lamr = T.mk_lambda_term "r" appy
	val appr = T.mk_apply_term lamr (T.mk_nuprl_small_integer_term 2)
...
val term23 =
	...
	val term = T.mk_nuprl_add_term t2 t3
	val name = "term23: 2 + 3"
    in (name, term)
    end

val bunch_of_terms = [term1,  term2,  term3,  term4,  term5,
		      term6,  term7,  term8,  term9,  term10,
		      term11, term12, term13, term14, term15,
		      term16, term17, term18, term19, term20,
		      term21, term22, term23]

val Evaluator1 = EV1.Evaluator1 (T.emlib (), T.mk_nuprl_simple_term "" [])

fun ev1 () = EV1.Evaluator1       (!mapref, T.mk_nuprl_simple_term "" [])
fun ev2 () = EV2.Evaluator2 false (!mapref, T.mk_nuprl_simple_term "" [])
fun ev3 () = EV3.Evaluator3 false (!mapref, T.mk_nuprl_simple_term "" [])
fun ev4 () = EV4.Evaluator4 false (!mapref, T.mk_nuprl_simple_term "" [])

fun eve2 () = EVE2.Evaluator2 false (!mapref, T.mk_nuprl_simple_term "" [])
...
fun terms2lib_rev [] = T.mk_nuprl_simple_term "!library" []
  | terms2lib_rev ((abs, wf) :: terms) =
    let val (cond, lhs, rhs) = T.dest_iabstraction abs
    in T.mk_nuprl_ref_simple_term "!library" [lhs, rhs, T.mk_rterm (terms2lib_rev terms)]
    end

fun terms2lib terms = terms2lib_rev (List.rev terms)

fun run_ev1 prt (SOME lib_file) ts steps term =
    let val terms = P.parse prt [] lib_file false (* false: no split *)
	val map   = T.terms2map terms
	val lib   = terms2lib ts
    in EV1.Evaluator1 (map, lib) steps term
    end
  | run_ev1 prt NONE ts steps term =
    let val lib = terms2lib ts
    in EV1.Evaluator1 (T.emlib (), lib) steps term
    end

fun run_ev2b prt (SOME lib_file) ts steps term =
    let val terms = P.parse prt [] lib_file false (* false: no split *)
	val map   = T.terms2map terms
	val lib   = terms2lib ts
    in EV2.Evaluator2 true (map, lib) steps term
    end
  | run_ev2b prt NONE ts steps term =
    let val lib = terms2lib ts
    in EV2.Evaluator2 true (T.emlib (), lib) steps term
    end

(*
fun terms2lib_closure_rev term [] = term
  | terms2lib_closure_rev ((abs, wf) :: terms) =
    let val (t1, lhs, rhs) = T.dest_iabstraction abs
	val _ = print (T.toStringTerm lhs
		       ^ "\n"
		       ^ T.toStringTerm rhs
		       ^ "\n")
	val cls = T.mk_nuprl_iclosure_term t e
    in T.mk_nuprl_simple_term "!library" [lhs, rhs, terms2lib_rev terms]
    end

fun terms2lib_closure terms = terms2lib_closure_rev (List.rev terms)
*)

(* evaluator -- recursive descent *)
fun run_ev1_map ts steps term =
    let val lib = terms2lib ts
    in EV1.Evaluator1 (!mapref, lib) steps term
    end

(* evaluator with closures *)
fun run_ev2_map ts steps term =
    let val lib = terms2lib ts
    in EV2.Evaluator2 false (!mapref, lib) steps term
    end

(* evaluator with closures
 * + closures instead of subst when unfolding *)
fun run_ev2b_map ts steps term =
    let val lib = terms2lib ts
    in EV2.Evaluator2 true (!mapref, lib) steps term
    end

(* evaluator with closures
 * + imperative environments *)
fun run_ev2c_map ts steps term =
    let val lib = terms2lib ts
    in EVE2.Evaluator2 false (!mapref, lib) steps term
    end

(* evaluator with closures
 * + closures instead of subst when unfolding
 * + CBV *)
fun run_ev2d_map ts steps term =
    let val lib = terms2lib ts
    in EV2.Evaluator2 true (!mapref, lib) steps term
    end

(* evaluator with continuations *)
fun run_ev3_map ts steps term =
    let val lib = terms2lib ts
    in EV3.Evaluator3 false (!mapref, lib) steps term
    end

(* evaluator with continuations
 * + closures instead of subst when unfolding *)
fun run_ev3b_map ts steps term =
    let val lib = terms2lib ts
    in EV3.Evaluator3 true (!mapref, lib) steps term
    end

(* evaluator with continuations
 * + datatype for the different sub-eval functions *)
fun run_ev3c_map ts steps term =
    let val lib = terms2lib ts
    in EV3.EvaluatorD3 false (!mapref, lib) steps term
    end

(* evaluator with continuations
 * + closures instead of subst when unfolding
 * + datatype for the different sub-eval functions *)
fun run_ev3d_map ts steps term =
    let val lib = terms2lib ts
    in EV3.EvaluatorD3 true (!mapref, lib) steps term
    end

(* evaluator with defunctionalization *)
fun run_ev4_map ts steps term =
    let val lib = terms2lib ts
    in EV4.Evaluator4 false (!mapref, lib) steps term
    end

(* evaluator with defunctionalization
 * + closures instead of subst when unfolding *)
fun run_ev4b_map ts steps term =
    let val lib = terms2lib ts
    in EV4.Evaluator4 true (!mapref, lib) steps term
    end

(* evaluator -- state machine *)
fun run_ev5_map ts steps term =
    let val lib = terms2lib ts
    in EV5.Evaluator5 false (!mapref, lib) steps term
    end

(* evaluator -- state machine
 * + closures instead of subst when unfolding *)
fun run_ev5b_map ts steps term =
    let val lib = terms2lib ts
    in EV5.Evaluator5 true (!mapref, lib) steps term
    end

fun start_session prt (SOME lib) =
    let val terms = P.parse prt [] lib false
	val map   = T.terms2map terms
	val _     = mapref := map
    in ()
    end
