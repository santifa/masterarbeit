...
fun updateUniEnvNam unifenv idor names =
	...
	val nam  = IDORMAP.unionWith (fn _ => raise EH.Impossible "") (nam1, nam2)
...
fun getUniEnvBindAtm atoms (E.BINDATM (atoms', lab, scheme)) kind =
  ...
  | getUniEnvBindAtm _ _ _ = NOT_FOUND 0
...
fun getUniEnvEnv _ (E.ENVVAR _) _ = SHADOWED
  ...
  | getUniEnvEnv id (E.ENVAPP (env1, env2)) f =
    (case getUniEnvEnv id env2 f of
	 SHADOWED    => SHADOWED
       | NOT_FOUND _ => getUniEnvEnv id env1 f
       | FOUND x     => FOUND x)
  ...
  | getUniEnvEnv _ (E.ENVMRK _) _ = NOT_FOUND 0
  | getUniEnvEnv _ E.ENVNUL _ = NOT_FOUND 0
  | getUniEnvEnv _ _ _ = raise EH.Impossible "Unexpected type environment in unification environment"
...
(* Gets the scheme associated to the nth binder of id from the beginning of the
 * the environment as oppposed of the other function which search from the end,
 * i.e., from the latest decs. *)
fun getScheme' unifenv id nb poly env =
    let fun getIdEnv nb (E.ENVAPP (env1, env2)) =
	    (case getIdEnv nb env1 of
		 FOUND (scheme, nb') =>
		 if nb' = 1
		 then FOUND (scheme, nb')
		 else if nb < 1
		 then raise EH.Impossible "We cannot search for the nth scheme for n < 1"
		 else getIdEnv (nb' - 1) env2
	       | NOT_FOUND n => getIdEnv n env2
	       | SHADOWED => SHADOWED)
	  | getIdEnv nb (E.ENVDEP (env, dep)) = getIdEnv nb env
	  | getIdEnv nb (E.ENVBIN (E.BINDVID (vid, lab, scheme), _)) =
	    if id = vid
	    then case scheme of
		     ...
		     in FOUND (E.VPOLY (sets', (cssch, ity'')), nb)
		     end
		   | E.VODEC ((ityvarset, etyvarset, idorset), (idor, ity)) =>
		     let val ity' = buildIty' unifenv ity ityvarset
		     in FOUND (E.VPOLY ((ityvarset, etyvarset, idorset), (E.mk_new_cs_scheme (), ity')), nb)
		     end
		   | E.VMONO (ityvar, deps, b) =>
		     ...
		     in FOUND (E.VPOLY (bound, (E.mk_new_cs_scheme (), ity2)), nb)
		     end
	    else NOT_FOUND nb
	  | getIdEnv nb (E.ENVBIN _) = NOT_FOUND nb
	  | getIdEnv nb (E.ENVVAR _) = SHADOWED
	  | getIdEnv nb (E.ENVMRK _) = NOT_FOUND nb
	  | getIdEnv nb (E.ENVNUL)   = NOT_FOUND nb
	  | getIdEnv _ _ = raise EH.Impossible "Unexpected solving environment"
    in case getIdEnv nb env of
	   FOUND (scheme, 1) => SOME scheme
	 | _ => NONE
    end
...
fun getScheme id nb poly file_base (SUCCESS (unifenv, envs)) =
    if nb < 1
    then raise EH.Impossible "We cannot search for the nth scheme for n < 1"
    else let val env = select_env file_base unifenv envs
	     (*val _   = print ("**********************\n")
	     ...
	     (*val _ = print ("\n----------------------------\n" ^
			    ...
			    (*"\n" ^ E.toStringEnv env ^*)
			    "\n----------------------------\n")*)
	     val schop = getScheme' unifenv id nb poly env
	     (*val _ = print "\n----------------------------\n"*)
	 in schop
	 end
  | getScheme id nb poly file_base (ERROR _) = NONE
...
fun matchIty tn (E.ITYVAR ityvar) c found =
    ...
    OR_FOUND_VAR
  | matchIty tn (E.ITYETV _) c found = raise EH.Impossible "We shouldn't have explicit type variables in overloaded types"
  | matchIty tn (ity as E.ITYCON (ityseq, itycon)) c found =
    matchItycon tn itycon c found ity
  (*| matchIty tn (E.ITYDPP (ity1, ity2)) c found = raise EH.Unimplemented ""*)
  | matchIty tn (E.ITYOR _) c found = raise EH.Impossible "we shouldn't have several layers of ORs"
...
fun orFullIty (ity as E.ITYVAR _) _ = NONE
  | orFullIty (ity as E.ITYETV _) _ = raise EH.Impossible "We shouldn't have explicit type variables in overloaded types"
  | orFullIty (ity as E.ITYCON (_, itycon)) n = orFullItycon itycon n ity
  | orFullIty (ity as E.ITYOR _) _ = raise EH.Impossible "we shouldn't have several layers of ORs"
    ...
    #1 (foldl (fn (ity, (NONE, n)) => (NONE, n + 1)
		| (ity, (SOME tcdeps1, n)) =>
		  (case orFullIty ity n of
		       NONE => (NONE, n + 1)
		     | SOME tcdeps2 =>
		       (SOME (FULL.unionWith (fn _ => raise EH.Impossible "") (tcdeps1, tcdeps2)), n + 1)))
	      (SOME FULL.empty, 0)
	      itys)
...
fun simpleOverloading ityseq1 ityseq2 paths =
	...
	fun genPairsIty (E.ITYCON (ityseq1, itycon1)) (E.ITYCON (ityseq2, itycon2)) =
	    ...
	    in case (itycon1', itycon2') of
		 ...
		 | _ => raise EH.Impossible "all dependencies have been stripped off"
	    end
	  ...
	  | genPairsIty ity (E.ITYOR (E.ORSEQ (ityseq, idor, lab))) =
	    ...
	    in case ityseq' of
		   E.ITYSEQVAR _ => NONE
		 | E.ITYSEQSEQ (itys, dep) => raise EH.Unimplemented ""
		 | E.ITYSEQDEP _ => raise EH.Impossible "all dependencies have been stripped off"
	    end
	  | genPairsIty (E.ITYOR (E.ORSEQ (ityseq, idor, lab))) ity = raise EH.Unimplemented ""
...
fun solvedEnv (E.ENVAPP (env, E.ENVMRK marker0)) marker =
  ...
  (* impossible cases: *)
  | solvedEnv (E.ENVVAR _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVVAR(solved)"
  | solvedEnv (E.ENVBIN _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVBIN(solved)"
  | solvedEnv (E.ENVDEP _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVDEP(solved)"
  | solvedEnv (E.ENVNUL)   _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVNUL(solved)"
  | solvedEnv (E.ENVFIL _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVFIL(solved)"
  | solvedEnv (E.ENVLOC _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVLOC(solved)"
  | solvedEnv (E.ENVCST _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVCST(solved)"
  | solvedEnv (E.ENVSUB _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVSUB(solved)"
  | solvedEnv (E.ENVPOL _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVPOL(solved)"
  | solvedEnv (E.ENVSET _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVSET(solved)"
  | solvedEnv (E.ENVERR _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVERR(solved)"
  | solvedEnv (E.ENVOVL _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVOVL(solved)"
  | solvedEnv (E.ENVTOF _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVTOF(solved)"
  | solvedEnv (E.ENVLAB _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVLAB(solved)"
  | solvedEnv (E.ENVOVE _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVOVE(solved)"
  | solvedEnv (E.ENVACC _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVACC(solved)"
...
fun getMonoTyvarsEnv unifenv (E.ENVVAR _) mrk = (true, false, initIEMon ())
  ...
  | getMonoTyvarsEnv _ (E.ENVFIL _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVFIL(mono)"
  | getMonoTyvarsEnv _ (E.ENVLOC _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVLOC(mono)"
  | getMonoTyvarsEnv _ (E.ENVCST _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVCST(mono)"
  | getMonoTyvarsEnv _ (E.ENVSUB _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVSUB(mono)"
  | getMonoTyvarsEnv _ (E.ENVPOL _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVPOL(mono)"
  | getMonoTyvarsEnv _ (E.ENVSET _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVSET(mono)"
  | getMonoTyvarsEnv _ (E.ENVERR _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVERR(mono)"
  | getMonoTyvarsEnv _ (E.ENVOVL _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVOVL(mono)"
  | getMonoTyvarsEnv _ (E.ENVTOF _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVTOF(mono)"
  | getMonoTyvarsEnv _ (E.ENVLAB _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVLAB(mono)"
  | getMonoTyvarsEnv _ (E.ENVOVE _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVOVE(mono)"
  | getMonoTyvarsEnv _ (E.ENVACC _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVACC(mono)"
...
fun getOverseqIty unifenv (E.ITYVAR _) = IDORMAP.empty
  ...
  | getOverseqIty unifenv (E.ITYOR (E.ORSEQ (ityseq, idor, lab))) =
    ...
    in case getUniEnvOr idor unifenv of
	   NONE => map
	 | SOME (paths, deps) => raise EH.Unimplemented ""
	...
	fun update E.ENVNUL unifenv = unifenv
	  ...
	  | update env unifenv =
	    (print (E.toStringEnv env);
	     raise EH.Impossible "The environment should be a constrained binder")
  ...
  | solveEnv unifenv deps (E.ENVTOF vid) _ =
    (if isFirst ()
     then case getUniEnvVid vid unifenv of
	      SHADOWED             => ()
	    | NOT_FOUND _          => print ("[" ^ vid ^ " not found]\n")
	    | FOUND (scheme, _, _) => print ("[" ^ vid ^ " has type " ^ E.ppScheme scheme ^ "]\n")
     else ();
     unifenv)
  | solveEnv unifenv deps (E.ENVMRK _) _ =
    raise EH.Impossible "marker environment should not be generated at inital constraint generation"
...
and solveAcc unifenv deps (E.ACCVID (vid, ityvar)) (D.DEPL label) =
  ...
  | solveAcc unifenv deps (E.ACCTYC (tycon, (sv, tv))) (D.DEPL label) =
    (case getUniEnvTyc tycon unifenv of
	 SHADOWED  => unifenv
       | NOT_FOUND _ => raise errorex (mk_pre_error (FREEID tycon) deps)
  ...
  | solveAcc unifenv deps (E.ACCTYV (tyvar, ityvar)) (D.DEPL label) =
    (case getUniEnvTyv tyvar unifenv of
	 SHADOWED  => unifenv
       | NOT_FOUND _ => raise errorex (mk_pre_error (FREEID tyvar) deps)
	 ...
	 in case ity' of
		...
		then unifenv
	      ...
	      | _ => raise EH.Impossible "explicit type variable binder should bind an implicit type variable"
	 end)
  | solveAcc unifenv deps (E.ACCATM (atoms, ityvar)) (D.DEPL label) =
    (case getUniEnvAtm atoms unifenv of
	 SHADOWED  => unifenv
       | NOT_FOUND _ => raise errorex (mk_pre_error (FREEATOMS atoms) deps)
       | FOUND (scheme, lab, _) =>
...
and solveSub unifenv deps (sub as E.SUBITY (E.ITYDEP (ity1, dep), ity2)) = solveSub unifenv (D.add (deps, dep)) (E.SUBITY (ity1, ity2))
  ...
  | solveSub unifenv deps (sub as E.SUBITY (ity1 as E.ITYCON (ityseq1, itycon1), ity2 as E.ITYCON (ityseq2, itycon2))) =
    ...
    in case (itycon1', itycon2') of
	 ...
	 | (E.ITYCONDEP _, _) => raise EH.Impossible "all the dependencies should have been removed"
	 | (_, E.ITYCONDEP _) => raise EH.Impossible "all the dependencies should have been removed"
    end
	 ...
	 NONE => let val ty1  = E.mk_new_tyvar ()
		 ...
		 in case seq2 of
			...
			in solveEnv unifenv deps (E.list2env [sub1, sub2, env]) D.dummy_dep
		      ...
		      | _ => raise  EH.Impossible "implication should have 2 parameters"
  ...
  | solveSub unifenv deps (sub as E.SUBIMP (E.ITYSEQSEQ (itys1, lab1), E.ITYSEQSEQ (itys2, lab2))) =
    (case (itys1, itys2) of
	 ...
	 in solveEnv unifenv deps env D.dummy_dep
       ...
       | _ => raise EH.Impossible "the arrow type should have 2 arguments")
...
and solveCst unifenv deps (E.CSITY (E.ITYDEP (ity1, dep), ity2)) = solveCst unifenv (D.add (deps, dep)) (E.CSITY (ity1, ity2))
  ...
  | solveCst unifenv deps (E.CSITY (ty1 as E.ITYOR (orseq as E.ORSEQ (ityseq1, id, lab)),
    ...
    let fun match restriction depsor =
		...
		fun matchTn (E.ITYCONVAR _) = (unifenv, deps, E.ENVNUL)
		  ...
		  | matchTn (E.ITYCONDEP _) = raise EH.Impossible "we striped the internal type constructor"
	    in matchTn itycon0
	    end
    in case getUniEnvOr id unifenv of
	   NONE =>
	...
	fun restrict restrict1 depsRes1 restrict2 depsRes2 =
		...
		in (* TODO: raising an error here is not the correct thing to do
		    ...
		    *)
		    ...
		    then let val cst      = E.CSITY ((fn [pair] => pair | _ => raise EH.Impossible "") (ListPair.zip (list1t, list2t)))
  ...
  | solveCst unifenv deps (E.CSITY (E.ITYETV (etyvar, lab), E.ITYCON (ityseq, itycon))) =
    ...
    in case itycon2 of
	 ...
	 (*| E.ITYCONFUN _ => raise EH.Impossible "type functions should have already been resolved"*)
	 | E.ITYCONDEP _ => raise EH.Impossible "all the dependencies should have been removed"
    end
...
fun isDumBind (E.BINDVID (_, _, E.VMONO (ityvar, _, _))) = (ityvar = E.ityvarFresh)
  ...
  | isDumBind (E.BINDTYC (_, _, (boud, (E.ITYSEQVAR ityseqvar, E.ITYVAR (ityvar, _))))) =
...
fun dum (E.ENVVAR envvar)    = E.ENVVAR E.envvarFresh
  ...
  | dum (E.ENVPOL _)         = raise EH.Impossible "pol environment should never be labelled"
  | dum (E.ENVAPP _)         = raise EH.Impossible "environment sequences should never be labelled"
  | dum (E.ENVSET _)         = raise EH.Impossible "environment sets should never be labelled"
  | dum (E.ENVLOC _)         = raise EH.Impossible "local environment should never be labelled"
  | dum (E.ENVDEP _)         = raise EH.Impossible "dependent environment should never be labelled"
  | dum (E.ENVFIL _)         = raise EH.Impossible "environment file should never be labelled"
  | dum (E.ENVERR _)         = E.ENVNUL
  | dum (E.ENVOVL _)         = E.ENVNUL
  | dum (E.ENVTOF _)         = E.ENVNUL
  | dum (E.ENVMRK _)         = raise EH.Impossible "marker environment should never be labelled"
  | dum (E.ENVLAB _)         = E.ENVNUL
  | dum (E.ENVOVE _)         = E.ENVNUL
  | dum E.ENVNUL             = raise EH.Impossible "empty environment should never be labelled"
...
fun filterEnv (E.ENVVAR _) deps1 deps2 = raise EH.Impossible "variable should always be labelled"
  | filterEnv (E.ENVBIN _) deps1 deps2 = raise EH.Impossible "binder should always be labelled"
  | filterEnv (E.ENVACC _) deps1 deps2 = raise EH.Impossible "accessor should always be labelled"
  | filterEnv (E.ENVCST (E.CSENV (ev, env))) deps1 deps2 = E.ENVCST (E.CSENV (ev, filterEnv env deps1 deps2))
  | filterEnv (E.ENVCST _) deps1 deps2 = raise EH.Impossible "equality constraint should always be labelled"
  | filterEnv (E.ENVSUB _) deps1 deps2 = raise EH.Impossible "subtyping constraint should always be labelled"
  ...
  | filterEnv (E.ENVERR _) _ _ = raise EH.Impossible "enverr should always be labelled"
  | filterEnv (E.ENVOVL _) _ _ = raise EH.Impossible "envovl should always be labelled"
  | filterEnv (E.ENVTOF _) _ _ = raise EH.Impossible "type requests should always be labelled"
  | filterEnv (E.ENVMRK _) _ _ = raise EH.Impossible "marker environment should not be generated at initial constraint generation"
  | filterEnv (E.ENVLAB _) _ _ = raise EH.Impossible "lab environment should always be labelled"
  | filterEnv (E.ENVOVE _) _ _ = raise EH.Impossible "ove environment should always be labelled"
...
fun get_extregs_file (term as A.N {kind, label, value, regions, parents, children}) error =
    ...
    else raise EH.Impossible "wrong format"
...
fun reduce2 env sub deps [] _ = deps
  | reduce2 env sub deps (x :: xs) prog =
	...
	val _ = print ("1: "  ^ A.export (slicing prog deps') ^ "\n")
	val _ = print ("removed dependency: " ^ D.toStringDep x ^ "\n")
	val _ = print "--\n"*)
...
(*fun minimize1 prog err env sub =
	...
	val _     = if isSuccess (solver (filterEnv env deps1 D.empty (*D.empty*)) sub)
		    then (print (formatError err prog);
			  ...
			  raise EH.Impossible "The error should be an error before minimisation")
		    else ()
	...
	val _     = if isSuccess (solver (filterEnv env deps2 D.empty (*D.empty*)) sub)
		    then (print (formatError err prog);
			  ...
			  raise EH.Impossible "The error should be an error during minimisation")
		    else ()
    ...
    in case solver env' sub of
	   SUCCESS _ => (print (formatError err prog);
			 ...
			 raise EH.Impossible "The error should be an error after minimisation")
	 | ERROR error => error
    end*)
...
((*printSearchSpace searchspace;*)
     ...
     then (errors, false)
	    ...
	    | NONE => case solver (filterEnv env labs filter (*D.empty*)) sub of
			  SUCCESS _ =>
...
fun noTyvarsInSolvedIty (E.ITYVAR (ityvar, eq)) ityvarset etyvarset deps prog prt =
  ...
  | noTyvarsInSolvedIty (E.ITYCON (ityseq, itycon)) ityvarset etyvarset deps prog prt =
    ...
    in case (ityseq', itycon') of
	 ...
	 | _ => raise EH.Impossible "We shouldn't have sequence of type constructor variables at this stage"
    end
  | noTyvarsInSolvedIty (E.ITYOR (E.ORSEQ (ityseq, _, _))) ityvarset etyvarset deps prog prt =
    ...
    in case ityseq0 of
	   E.ITYSEQSEQ (itys, _) =>
	   List.concat (map (fn ity => noTyvarsInSolvedIty ity ityvarset etyvarset deps' prog prt) itys)
	 | _ => raise EH.Impossible "We shouldn't have sequence variables at this stage"
    end
...
fun noTyvarsInSolvedEnv unifenv envs prog prt =
    let fun getErrors (E.ENVVAR _) deps = raise EH.Impossible "If program is typable, we shouldn't get any environment variable"
	  ...
	  | getErrors (E.ENVFIL _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVFIL(tv)"
	  | getErrors (E.ENVLOC _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVLOC(tv)"
	  | getErrors (E.ENVCST _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVCST(tv)"
	  | getErrors (E.ENVSUB _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVSUB(tv)"
	  | getErrors (E.ENVPOL _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVPOL(tv)"
	  | getErrors (E.ENVSET _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVSET(tv)"
	  | getErrors (E.ENVERR _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVERR(tv)"
	  | getErrors (E.ENVOVL _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVOVL(tv)"
	  | getErrors (E.ENVTOF _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVTOF(tv)"
	  | getErrors (E.ENVLAB _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVLAB(tv)"
	  | getErrors (E.ENVOVE _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVOVE(tv)"
	  | getErrors (E.ENVACC _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVACC(tv)"
...
fun noOverloadedTopDec unifenv prog prt =
    let fun getErrors (E.ENVVAR _) deps = raise EH.Impossible "If program is typable, we shouldn't get any environment variable"
	  ...
	  | getErrors (E.ENVMRK _) _ = []
	  | getErrors E.ENVNUL _ = []
	  | getErrors (E.ENVFIL _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVFIL(ove)"
	  | getErrors (E.ENVLOC _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVLOC(ove)"
	  | getErrors (E.ENVCST _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVCST(ove)"
	  | getErrors (E.ENVSUB _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVSUB(ove)"
	  | getErrors (E.ENVPOL _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVPOL(ove)"
	  | getErrors (E.ENVSET _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVSET(ove)"
	  | getErrors (E.ENVERR _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVERR(ove)"
	  | getErrors (E.ENVOVL _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVOVL(ove)"
	  | getErrors (E.ENVTOF _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVTOF(ove)"
	  | getErrors (E.ENVLAB _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVLAB(ove)"
	  | getErrors (E.ENVOVE _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVOVE(ove)"
	  | getErrors (E.ENVACC _) _ = raise EH.Impossible "Unexpected type environment in unification environment:ENVACC(ove)"
...
fun slice inputs outputop libop timelimit (prt as (prt1, prt2)) sub san btyp =
    ...
    in if btyp andalso san andalso b (* b means no errors *)
       then let val solved = solver env sub
	    in case solved of
		 ...
		 | ERROR error => raise EH.Impossible "the piece of code should be typable"
	    end
    ...
    handle EH.Impossible    msg => (genFinished (finishedLispMessage2 msg); raise EH.Impossible msg)
	 | EH.Unimplemented msg => (genFinished (finishedLispMessage2 msg); raise EH.Unimplemented msg)
