...
<INITIAL> "@|"               => (token T.ATPIPE     yytext arg);
<INITIAL> "^|"               => (token T.LIFTPIPE   yytext arg);
...
<INITIAL> "base"             => (token T.BASE       yytext arg);
...
<INITIAL> "msg"              => (token T.CONSMSG    yytext arg);
<INITIAL> "send"             => (token T.SEND       yytext arg);
<INITIAL> "broadcast"        => (token T.BROADCAST  yytext arg);
