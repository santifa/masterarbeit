...
fun newLineCol (line, col) =
	...
	(case LD.getLength (line - 1) of
	     NONE => raise EH.Impossible "The parser failed on somewhere outside the text of the file"
	   | SOME c => (line - 1, c))
      | SOME c =>
	if c = 0
...
fun dummyparsing file messages =
    let val errs = map (fn (msg, _, regs) =>
			   let val regs' =
				   map (fn reg =>
					   let val pos1 = newLineCol (R.getFrom reg)
...
fun parseStream file inputStream =
	...
	val ast = astFunction ()
    in if !error
       then dummyparsing file (!messages) (*raise ParsingError (n, (!messages))*)
  ...
  | merge (prog :: progs) =
    ...
    in if A.getKind prog = (A.PROG, A.PROG_P)
	  ...
	  A.getKind prog' = (A.PROG, A.PROG_P)
       ...
       else raise EH.Impossible "merge programs"
    end
...
fun close_inputs b [] = []
  | close_inputs b (input :: inputs) =
	...
	val progs = map (fn file => parseGen b (file ^ ".esh")) files
