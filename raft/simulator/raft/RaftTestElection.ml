...
class ['a, 'b, 'c, 'd] raft c t = object(self)
  ...
  method change_dst dm ids = cdest dm ids
  ...
  | dm :: dms ->
    ...
    | id :: ids ->
      new_round dm;
      let is_last_round = !rounds / 2 in
      if max_rounds <= is_last_round then
        ([], []) (* stop after n rounds with success *)
      else begin
...
let election (responses : directedMsgs) : directedMsgs =
  match responses with
  | [] -> log_test_success ("Run " ^ (Int.to_string (!rounds / 2)) ^ " of election."); []
  | dm :: msgs ->
    match (Obj.magic dm.dmMsg) with
...
let _ =
  let t = [("Test Election", election)] in
  let c = new raft {
    version = "1.0";
    protocol = "Raft";
    client_id = 0;
