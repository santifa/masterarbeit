...
Section PrimaryBackup.
    ...
    MkPBcontext
      {
        ...
        PBmax_in_progress : nat;

        (* This number has to be big enough so that replicas do not stall waiting
         for a checkpoint to become stable *)
        PBwater_mark_range : nat; (* this should be SeqNum type *)

        (* usually half of the water-mark range *)
        PBcheckpoint_period : nat;

        PBdigest : Set;
        PBdigestdeq : Deq PBdigest;

        PBtoken    : Set;
        PBtokendeq : Deq PBtoken;
        ...
        (* ++++++++ replicated service ++++++++ *)
        PBoperation : Set;
        PBopdeq : Deq PBoperation;

        PBresult : Set;
        PBresdeq : Deq PBresult;

        PBsm_state : Set;
        PBsm_initial_state : PBsm_state;
        PBsm_update : Client -> PBsm_state -> PBoperation -> PBresult * PBsm_state;

        PBtimer_delay : nat;
      }.

  Context { pb_context : PBcontext }.
  ...
  Definition PBtokens := list PBtoken.
    ...
    MkLastReplyEntry
      {
        ...
        lre_reply     : option PBresult; (* None is the initial value *)
      }.
  ...
  Class PBhash :=
    MkPBhash
      {
        create_hash_messages : list PBmsg -> PBdigest;
        verify_hash_messages : list PBmsg -> PBdigest -> bool;

        create_hash_state_last_reply  : PBsm_state -> LastReplyState -> PBdigest;
        verify_hash_state_last_reply  : PBsm_state -> LastReplyState -> PBdigest -> bool;
      }.

  Context { pb_hash : PBhash }.
